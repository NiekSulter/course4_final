...
def database():
    ...
    list_lineage = []
    list_description = []
    mysql_insert_query = """select protein_name, organism_name, accession, e_value, percent_identity, query_cover, lineage
    from protein_seq left outer join taxonomy t on protein_seq.taxonomy_tax_id = t.tax_id"""
        ...
        if checkbox_s:
            if checkbox_f:
                if checkbox_o:# p:True s:True f:True o:True
                    ...
                    where protein_name like %s or lineage like %s or description like %s or organism_name like %s;"""
                ...
                if checkbox_o: # p:True s:True f:False o:True
                    mysql_insert_query = """select protein_name, organism_name, accession, e_value, percent_identity, query_cover, lineage 
                    from protein_seq join taxonomy t on protein_seq.taxonomy_tax_id = t.tax_id
                    where protein_name like %s or lineage like %s or organism_name like %s;"""
            ...
            if checkbox_f:
                if checkbox_o: # p:True s:False f:True o:True
                    mysql_insert_query = """select protein_name, organism_name, accession, e_value, percent_identity, query_cover, lineage 
                                        from protein_seq join taxonomy t on protein_seq.taxonomy_tax_id = t.tax_id
                                        where protein_name like %s or description like %s or organism_name like %s;"""
                    query_input = ("%" + filter + "%", "%" + filter + "%", "%" + filter + "%")
                ...
                if checkbox_o: # p:True s:False f:False o:True
                    mysql_insert_query = """select protein_name, organism_name, accession, e_value, percent_identity, query_cover, lineage 
                                        from protein_seq join taxonomy t on protein_seq.taxonomy_tax_id = t.tax_id 
                                        where protein_name like %s or organism_name like %s;"""
                    query_input = ("%" + filter + "%", "%" + filter + "%")
        ...
        if checkbox_s:
            if checkbox_f:
                if checkbox_o: # p:False s:True f:True o:True
                    mysql_insert_query = """select protein_name, organism_name, accession, e_value, percent_identity, query_cover, lineage 
                                        from protein_seq join taxonomy t on protein_seq.taxonomy_tax_id = t.tax_id
                                        where lineage like %s or description like %s or organism_name like %s;"""
                    query_input = ("%" + filter + "%", "%" + filter + "%", "%" + filter + "%")
...
def userblast(jid):
    ...
    conn = mysql.connector.connect(
        ...
        db="user_blast")
    cursor = conn.cursor()
    cursor.execute("select header, dna_seq from seq where job_id like \'{}\'".format(jid))
    for i in cursor:
        header = i[0]
        sequence = i[1]
    cursor.close()
    cursor2 = conn.cursor()
    cursor2.execute("select protein_name, organism_name, accession, e_value, percent_identity, query_cover, job_job_id from protein_seq where job_job_id like \'{}\'".format(jid))
    for i in cursor2:
        list_all.append(i)
    conn.close()
